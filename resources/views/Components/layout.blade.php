...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full bg-gray-100">
    <head>
        ...
        {{-- Now, we can use HMR for CSS and JS --}}
      ...
      <div class="min-h-full">
            ...
            <div class="flex h-16 items-center justify-between">
              ...
              <div class="hidden md:block">
                <div class="ml-4 flex items-center md:ml-6">
                  @guest
                    {{-- Versus @auth directive --}}
                    <x-nav-link href="/login" :active="request()->is('login')">Log In</x-nav-link>
                    <x-nav-link href="/register" :active="request()->is('register')">Register</x-nav-link>
                  @endguest
                  @auth
                    {{-- Always use POST for logout --}}
        ...
        <header class="bg-white shadow">
          <div class="mx-auto max-w-7xl px-5 py-6 sm:px-6 lg:px-8 sm:flex sm:justify-between">
            <h1 class="text-3xl font-bold tracking-tight text-custom-color">{{ $heading }}</h1>
